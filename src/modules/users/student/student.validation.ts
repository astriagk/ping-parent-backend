...
export const adminCreateStudentSchema = Joi.object({
  school_id: Joi.string().required().messages({
    "any.required": VALIDATION_MESSAGES.STUDENT.SCHOOL_ID_REQUIRED,
  }),
  student_name: Joi.string().min(2).max(100).required().messages({
...
export const adminBulkCreateStudentsSchema = Joi.object({
  students: Joi.array()
    .items(adminCreateStudentSchema)
    .min(1)
    .max(50)
    .required(),
});

export const adminUpdateStudentSchema = Joi.object({
  ...
  emergency_contact: Joi.string()
    .pattern(/^[+]?[0-9]{10,15}$/)
    .optional(),
  medical_info: Joi.string().max(500).optional().allow(""),
}).min(1);
